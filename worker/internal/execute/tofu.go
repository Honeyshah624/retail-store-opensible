...
func tofuArgs(action string) ([]string, error) {
	...
	return nil, errors.New(
		"unsupported tofu action: " + action,
	)
}
...
func executeTofuRun(executionID string, execData map[string]any, projectID string, client *httpclient.Client) {
	...
	// For apply, require the exact saved plan produced by the plan step.
	// This prevents apply from recalculating infrastructure after approval.
	if strings.EqualFold(action, "apply") {
		planPath := filepath.Join(stackDir, "tfplan")

		info, err := os.Stat(planPath)
		if err != nil {
			if os.IsNotExist(err) {
				sendLog(
					"ERROR: approved OpenTofu plan 'tfplan' was not found.\n" +
						"Run the plan step successfully before approving apply.\n",
				)
			} else {
				sendLog(
					fmt.Sprintf(
						"ERROR: unable to inspect approved plan %s: %v\n",
						planPath,
						err,
					),
				)
		...
		if info.IsDir() {
			sendLog(
				"ERROR: expected 'tfplan' to be a saved OpenTofu plan file, " +
		...
		if info.Size() == 0 {
			sendLog(
				"ERROR: approved OpenTofu plan 'tfplan' is empty.\n",
			)
